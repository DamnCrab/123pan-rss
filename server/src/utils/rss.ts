 ...
 * @returns 下载创建结果
 */
export async function createMagnetDownload(env: any, magnetLinkId: number): Promise<MagnetDownloadResult> {
    const database = db(env)
    ...
    try {
        ...
        // 调用115网盘API创建离线下载任务
        const response = await fetch('https://webapi.115.com/web/lixian/?ct=lixian&ac=add_task_url', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Cookie': env.COOKIE_115
            },
            body: new URLSearchParams({
                url: link.magnetLink,
                wp_path_id: env.WP_PATH_ID || '0'
            })
        })
        ...
        if (result.state) {
            // 创建成功
            const taskId = result.info?.[0]?.info_hash || 'unknown'
            ...
            console.log(`磁力链接 ${magnetLinkId} 下载任务创建成功, 任务ID: ${taskId}`)
            ...
            // 创建失败
            const errorMsg = result.error || result.error_msg || '创建下载任务失败'
            
            await database
                .update(magnetLinksTable)
                .set({
                    downloadStatus: 'failed',
                    downloadFailReason: errorMsg,
                    downloadCreatedAt: now,
                    updatedAt: now
                })
                .where(eq(magnetLinksTable.id, magnetLinkId))

            console.error(`磁力链接 ${magnetLinkId} 下载任务创建失败:`, errorMsg)
 ...
 * @returns 重试结果
 */
export async function retryMagnetDownload(env: any, magnetLinkId: number): Promise<MagnetRetryResult> {
    const database = db(env)
...
/**
 * 兼容旧版本的processRSSFeeds函数
 * @deprecated 请使用 updateAllRSS 函数
 */
export async function processRSSFeeds(env: any, forced = false) {
    console.warn('processRSSFeeds 函数已废弃，请使用 updateAllRSS 函数')
    const result = await updateAllRSS(env, forced)
    return result
}
