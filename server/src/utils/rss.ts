...
import {createOfflineDownload} from "./cloud123";
...
export async function createMagnetDownload(env: any, magnetLinkId: number): Promise<{ success: boolean; magnetLinkId: number; error: string; taskId: any }> {
    ...
    try {
        ...
        // 直接调用createOfflineDownload，不再在其内部查询数据库
...
export async function downloadAllPendingMagnets(env: any, concurrencyLimit: number = 3, rssId?: number): Promise<{
    ...
    try {
        ...
        // 直接获取所有磁力链接对应的RSS订阅信息，避免重复查询
        const magnetLinksWithRss = await database
            .select({
                magnetLink: magnetLinksTable,
                rssSubscription: rssSubscriptionsTable
            })
        ...
        console.log(`找到 ${magnetLinksWithRss.length} 个待下载的磁力链接`)
        
        if (magnetLinksWithRss.length === 0) {
            return {
                total: 0,
                success: 0,
                failed: 0,
        ...
        // 分批处理，避免并发过多
        for (let i = 0; i < magnetLinksWithRss.length; i += concurrencyLimit) {
            const batch = magnetLinksWithRss.slice(i, i + concurrencyLimit)
            
            const batchPromises = batch.map(async (item) => {
                const { magnetLink: magnet, rssSubscription } = item
                try {
                    // 检查是否已经有下载任务
                    if (magnet.downloadTaskId) {
                        skippedCount++
                        return {
                    ...
                    if (task) {
                        ...
                        successCount++
                        console.log(`磁力链接 ${magnet.id} 下载任务创建成功，taskId: ${taskId}`)
                        ...
                        failedCount++
                        console.log(`磁力链接 ${magnet.id} 下载任务创建失败: 创建下载任务失败，未返回taskId`)
                    ...
                    try {
                        await database
                            ...
                            .set({
                                downloadStatus: 'failed',
                                downloadFailReason: errorMessage,
                                downloadCreatedAt: Date.now(),
                            })
                            .where(eq(magnetLinksTable.id, magnet.id))
                    } catch (updateError) {
            ...
            if (i + concurrencyLimit < magnetLinksWithRss.length) {
                await new Promise(resolve => setTimeout(resolve, 1000))
            }
        }
        
        console.log(`批量下载完成: 总计 ${magnetLinksWithRss.length}, 成功 ${successCount}, 失败 ${failedCount}, 跳过 ${skippedCount}`)
        
        return {
            total: magnetLinksWithRss.length,
