...
import {searchQuerySchema, idQuerySchema, timeUnitSchema, paginatedResponseSchema, paginationQuerySchema} from "../utils/openApiSchemas";
...
app.get('/',
    describeRoute({
        tags: ['RSS订阅'],
        summary: '获取RSS订阅列表',
        description: '获取当前用户的所有RSS订阅列表，支持分页和按名称搜索',
        security: [{bearerAuth: []}],
        responses: responseSchema(paginatedResponseSchema(rssSubscriptionSchema))
    }),
    zValidator('query', searchQuerySchema.merge(paginationQuerySchema)), async (c) => {
        try {
            const user = c.get('jwtPayload')
            const database = db(c.env)
            ...
            const subscriptions = await database
                ...
                .from(rssSubscriptionsTable)
                .where(whereCondition)
                .orderBy(desc(rssSubscriptionsTable.createdAt))
                .limit(pageSize)
                .offset((page - 1) * pageSize)

            return c.json({
                success: true,
                message: search ? `搜索到${total}个相关订阅` : '获取RSS订阅列表成功',
                data: {
                    list: subscriptions,
                    total,
                    page,
